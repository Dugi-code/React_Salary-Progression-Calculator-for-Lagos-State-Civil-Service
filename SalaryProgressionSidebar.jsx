...
export default function SalaryProgressionSidebar({
  ...
  // Handle View Entries button click - no password required
  function handleViewEntriesClick() {
    // Explicitly ensure no password requirement for "DCode" or any other password
    console.log("[SalarySidebar] View Entries accessed - no password required");
  ...
  async function handleCompute() {
    ...
    try {
      ...
      if (Array.isArray(employees) && employees.length > 0 && Array.isArray(data.results) && data.results.length > 0) {
        ...
        setShowReport(true); // Auto-show report when results are available
      } else {
      ...
      {results && (
        <div style={defaultStyles.results}>
          <div style={{ 
            ...
            <strong>Progression Results:</strong>
            <button
              onClick={() => setShowRawResults(!showRawResults)}
              style={{
                padding: '6px 12px',
                fontSize: '12px',
                backgroundColor: showRawResults ? '#007bff' : '#f8f9fa',
                color: showRawResults ? 'white' : '#333',
              ...
              onMouseEnter={(e) => {
                e.target.style.backgroundColor = showRawResults ? '#0056b3' : '#e9ecef';
                e.target.style.transform = 'translateY(-1px)';
              }}
              onMouseLeave={(e) => {
                e.target.style.backgroundColor = showRawResults ? '#007bff' : '#f8f9fa';
            ...
            >
              {showRawResults ? 'Hide' : 'Show'} Results
            </button>
          </div>
          ...
          {reportContext && (
            <div style={{
              ...
              <div style={{
                ...
                <strong style={{ color: '#495057' }}>View Entries:</strong>
                <button
                  onClick={handleViewEntriesClick}
                  style={{
                    padding: '6px 12px',
                    fontSize: '12px',
                    backgroundColor: showReport ? '#28a745' : '#f8f9fa',
                    color: showReport ? 'white' : '#333',
                  ...
                  onMouseEnter={(e) => {
                    e.target.style.backgroundColor = showReport ? '#1e7e34' : '#e9ecef';
                    e.target.style.transform = 'translateY(-1px)';
                  }}
                  onMouseLeave={(e) => {
                    e.target.style.backgroundColor = showReport ? '#28a745' : '#f8f9fa';
                    e.target.style.transform = 'translateY(0)';
                  }}
                  onFocus={(e) => {
                    e.target.style.outline = '2px solid #28a745';
                    e.target.style.outlineOffset = '2px';
                  }}
                  onBlur={(e) => {
                    e.target.style.outline = 'none';
                  }}
                  title="View Entries - No Password Required"
                >
